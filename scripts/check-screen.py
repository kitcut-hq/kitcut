...
def pii_rules():
    ...
    cases = [
        ...
        ("pMEAHyrOCbAo@nGaRbLeD.FoTOBMionJaTMTW10AWTqyWxKHMKOK3caiT", "email", False),
    ...
    check("Luhn rejects the same with one digit mistyped", sp.luhn("4111111111111112"), False)
    ...
    check(
        "its rect unions both, plus the pad",
        [round(v, 3) for v in first["rect"]],
        [0.288, 0.388, 0.144, 0.064],
    )
...
def cut_arithmetic():
    ...
    check(
        "nothing leaks past the ends", bool(sc.hold_on(np.zeros(10, bool), 2.0, 1.0).any()), False
    )
    ...
    check("a 1 s keep island inside a long wait joins the wait", sorted(set(out.tolist())), [1])

    print("\nscreen-cut: air is handed back at 1x, not sped")
    lab = np.array([2] * 10 + [0] * 40 + [2] * 10, np.int8)
    out = sc.smooth(lab, fps=2.0, cfg=dict(sc.DEFAULTS, air=1.0, min_drop=1.0, min_keep=0.5))
    check(
        "the sample either side of a dropped run is keep, not speed",
        (int(out[10]), int(out[59])),
        (2, 2),
    )
    check("the middle of the dropped run is still dropped", int(out[35]), 0)

    print("\nscreen-cut: segments and speeds")
    segs = sc.segments(np.array([2] * 10 + [0] * 10 + [1] * 10, np.int8), fps=2.0, dur=15.0)
    check("three runs become three segments", len(segs), 3)
    check(
        "boundaries land on the sample grid",
        [(a, b, c) for a, b, c in segs],
        [(0.0, 5.0, 2), (5.0, 10.0, 0), (10.0, 15.0, 1)],
    )
...
def pipeline_pieces():
    ...
    frames = [
        ...
        },
        {
            "t": 8.0,
            "carried": True,
            "lines": [{"box": [0.1, 0.2, 0.1, 0.02], "text": "+38 (066) 431-4978", "conf": 0.9}],
        },
    ]
    ...
    check(
        "a labelled line needs only its digits covered",
        tb.recall(half, [pii2], "fake/src.mp4", info, [], {"phone"})[0]["phone:0661234567"][1],
        1,
    )
    check(
        "...but a bare number still needs most of its box",
        tb.recall(half, [pii], "fake/src.mp4", info, [], {"phone"})[0]["phone:0661234567"][1],
        0,
    )
    ...
    plan = {
        "path": __file__,
        "segments": [{"start": 0, "end": 1, "speed": 1.0, "out": 1}],
        "src": {"blur": []},
    }
    ...
    check(
        "a blur rect changes the key",
        a == sc.piece_key(dict(plan, src={"blur": [{"rect": [0, 0, 1, 1]}]}), dict(sc.DEFAULTS)),
        False,
    )
...
def graph_blurs_after_the_cut():
    ...
    check(
        "untracked: the mask is a black frame derived from the video",
        any("drawbox=x=0:y=0:w=iw:h=ih:color=black" in x for x in parts2),
        True,
    )
    check("untracked: no second input is referenced", any("[1:v]" in x for x in parts2), False)
    check(
        "untracked: still one gaussian, after the cut",
        [
            i > next(i for i, x in enumerate(parts2) if "concat=n=3" in x)
            for i, x in enumerate(parts2)
            if "gblur=sigma=3.0" in x
        ],
        [True],
    )
    ...
    check(
        "nothing to redact: no gaussian at all", any("gblur=sigma=3.0" in x for x in parts3), False
    )
...
def frame_change_matches_numpy():
    ...
    same = all(
        (tb.frame_change(b, a) < tb.CHANGE_SKIP) == (ref(b, a) < tb.CHANGE_SKIP)
...
def film_time_redaction():
    ...
    hand = [
        {"rect": [0.1, 0.2, 0.3, 0.05], "when": [10.0, 20.0], "why": "art"},
    ...
    check("a state STRADDLING the start is covered", len(fr.hand_boxes(hand, 9.5, 10.5)), 2)
    check("a state ending exactly at t0 is not", len(fr.hand_boxes(hand, 5.0, 10.0)), 1)
    check("a rect with no window covers the whole film", len(fr.hand_boxes(hand, 999.0, 1000.0)), 1)
    check(
        "the tile carries its reason, never the secret",
        fr.hand_boxes(hand, 12.0, 13.0)[0]["text"],
        "art",
    )
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        check(
            "...and only the first state is still blurred",
            (runs[0]["i0"], runs[0]["i1"], len(runs[0]["key"])),
            (0, 29, 1),
        )
        check("...leaving the rest clear", len(runs[1]["key"]), 0)
        runs, _ = fr.mask_runs(states, per, {"0": "clear", "1": "clear"}, info, td)
        ...
        check(
            "a hand rect outlives the clear that dropped the detection",
            [len(r["key"]) for r in runs],
            [2, 1],
        )
        runs, _ = fr.mask_runs(states, per, {"0": "clear", "1": "clear"}, info, td, hand=hand)
        check("...and a review cannot clear it away at all", [len(r["key"]) for r in runs], [1])
