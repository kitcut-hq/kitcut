...
def _merged(preset, spec, key):
    ...
    for src, where in ((preset, "preset"), (spec, "overlay entry")):
        ...
        if not isinstance(block, dict):
            raise SystemExit("%s: %r should be a block of settings, got %r" % (where, key, block))
        out.update(block)
    return out
...
def source_png(spec, preset, tmpdir, tag, i):
    ...
    if len(given) != 1:
        raise SystemExit(
            "image overlay %d: give exactly one of %s -- got %s"
    ...
    if spec.get("card"):
        ...
        if not os.path.exists(page) or os.path.getmtime(page) < os.path.getmtime(card):
...
def wipe_alpha(at, dur, direction, feather, out=False):
    ...
    return "geq=a='alpha(X,Y)*%s':r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':enable='%s'" % (
        ramp,
        esc("between(t,%.3f,%.3f)" % (at, at + dur)),
    )
...
def treatment_chain(base, cfg, at, end, in_s, out_s, tag, i):
    ...
    if float(cfg.get("desaturate", 0)) > 0:
        steps.append("hue=s=%.3f:enable='%s'" % (1.0 - float(cfg["desaturate"]), win))
    ...
    if out_s > 0:
        steps.append("fade=t=out:st=%.3f:d=%.3f:alpha=1" % (max(at, end - out_s), out_s))
...
def prepare(
    preset_path, specs, vid_w, vid_h, tmpdir, tag="", base="0:v", first_input=1, runtime=None
):
    ...
    for i, spec in enumerate(specs):
        ...
        a_in = (
            _merged(anim, spec, "in")
            if isinstance(anim.get("in"), dict)
            else dict(spec.get("in") or {})
        )
        a_out = (
            _merged(anim, spec, "out")
            if isinstance(anim.get("out"), dict)
        ...
        if spec.get("background") is not None:
            ...
            bg_parts, cur = treatment_chain(
                ...
                float(bg.get("in_s", in_d)),
                float(bg.get("out_s", 0.0) if out_t != "none" else 0.0),
                tag,
                i,
            )
        ...
        if in_t == "wipe":
            steps.append(
                wipe_alpha(at, in_d, a_in.get("direction", "left"), a_in.get("feather_px", 24))
            )
        elif in_t == "fade":
        ...
        if out_t == "wipe":
            steps.append(
                wipe_alpha(
                    ...
                    a_out.get("feather_px", 24),
                    out=True,
                )
            )
        elif out_t == "fade":
            steps.append("fade=t=out:st=%.3f:d=%.3f:alpha=1" % (max(at, end - out_d), out_d))
        ...
        xe, ye = "%d" % x, "%d" % y
        if in_t == "slide":
            xe, ye = slide_xy(x, y, at, in_d, a_in.get("direction", "left"), vid_w, vid_h, iw, ih)
        if out_t == "slide":
            xe, ye = slide_xy(
                x,
                y,
        ...
        parts.append(
            "[%s][%s]overlay=x=%s:y=%s:format=auto:shortest=1"
            ":enable='%s'[%s]"
            % (cur, lbl, esc(xe), esc(ye), esc("between(t,%.3f,%.3f)" % (at, end)), nxt)
        )
        cur = nxt
...
def describe(spec, preset, runtime=None):
    ...
    src = os.path.basename(spec.get("image") or spec.get("html") or spec.get("card") or "?")
    ends = "to the end" if spec.get("dur") is None and runtime is not None else "for %.1fs" % dur
    s = "image overlay '%s' %s in at %.1fs %s" % (src, (a_in.get("type") or "fade"), at, ends)
    if spec.get("background") is not None:
        bg = _merged(preset, spec, "background")
        bits = []
...
def load_specs(args):
    ...
    spec = {"at": args.at}
    for key, val in (("image", args.image), ("html", args.html), ("card", args.card)):
        if val:
            spec[key] = val
...
def main():
    ...
    ap.add_argument(
        "--card", help="a card spec to design, render and overlay (see make-card.py --list)"
    )
    ap.add_argument(
        "--at",
        type=float,
    ...
    ap.add_argument("--dur", type=float, help="how long it stays; default to the end of the film")
    ap.add_argument(
        "--anim",
        choices=("wipe", "fade", "slide", "none"),
        help="entrance animation; default comes from the preset",
    )
    ap.add_argument(
        "--background", action="store_true", help="treat the film under it (B&W, blur, dim)"
    )
    ...
    ap.add_argument(
        "--png-only", action="store_true", help="prepare the PNGs and stop, to eyeball the artwork"
    )
    ap.add_argument(
        "--frame",
        type=float,
    ...
    ap.add_argument(
        "--clip", action="store_true", help="encode only the overlay windows, not the whole film"
    )
    ...
    if args.png_only:
        pngs, _, _ = prepare(
            ...
            args.height,
            args.tmp,
            runtime=0.0 if args.at >= 0 else 600.0,
        )
        for p in pngs:
    ...
    for i, s in enumerate(specs):
        ...
        if at >= dur:
            sys.exit("overlay %d starts at %.1fs but the film is %.1fs long" % (i, at, dur))
        print("  %s" % describe(s, preset, dur))

    pngs, fc, out_label = prepare(args.preset, specs, w, h, args.tmp, runtime=dur)
    ...
    if args.frame is not None:
        png = os.path.join(
            args.tmp,
            os.path.basename(os.path.splitext(args.video)[0]) + "-overlay-%.0fs.png" % args.frame,
        )
