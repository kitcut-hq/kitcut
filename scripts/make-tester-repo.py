...
SHIP_FILES = [
    ...
    "CLAUDE.md",
    "docs/reference.md",
    "config/caption-style.json",
    "config/elevenlabs-voices.json",
    "docs/known-issues.md",
    "docs/ffmpeg-recipes.md",
    "docs/karaoke-captions.md",
    "docs/retro-books-giveaway.md",
]
...
SKIP = [
    ...
    "make-tester-repo.py",
    "make-tester-history.py",
    "config/tester",
]  # tester/ lands via SHIP_AS, not as itself
...
NEVER_IN_HISTORY = [
    ...
    "docs/market-shorts-2026.md",
    "docs/shorts-strategy.md",
    "docs/shorts-gtm-playbooks.md",
...
def skipped(rel):
    parts = rel.replace("\\", "/")
    return any(
        fnmatch.fnmatch(os.path.basename(parts), pat) or parts == pat or parts.startswith(pat + "/")
        for pat in SKIP
    )
...
def scrub_claude_md(text):
    for old, new in SCRUB_CLAUDE_MD:
        if old not in text:
            sys.exit(
                "SCRUB FAIL: CLAUDE.md no longer contains the expected "
...
def drop_dev_only(text, name):
    ...
    while True:
        ...
        if b < 0:
            sys.exit("SCRUB FAIL: %s opens a dev-only region and never closes it" % name)
...
def build(outdir, pairs):
    ...
    if os.path.exists(outdir):
        ...
        if extra:
            sys.exit(
                "refusing to build into non-empty %s (found %s) -- "
                "delete it first, or point --out somewhere fresh" % (outdir, ", ".join(extra[:5]))
            )
...
def verify(outdir):
    ...
    for rel in NEVER_IN_HISTORY + [
        ...
        if os.path.exists(p) and (
            rel != "projects" or [x for x in os.listdir(p) if x != ".gitkeep"]
        ):
    ...
    for cur, dirs, files in os.walk(outdir):
        ...
        for f in files:
            ...
            for bad in FORBIDDEN:
                if bad in text:
                    problems.append("%s mentions %r" % (os.path.relpath(p, outdir), bad))
            ...
            if f.endswith(".md") and "dev-only" in text:
                problems.append("%s still carries a dev-only marker" % os.path.relpath(p, outdir))
...
def main():
    ...
    if args.list or not args.out:
        ...
        print(
            ...
            "empty: %s" % (len(pairs), total / 1e6, len(SCRUB_CLAUDE_MD), " ".join(EMPTY_DIRS))
        )
        print(
            "never ships: docs/product-strategy.md docs/market-shorts-2026.md "
    ...
    print("next: git init + commit there, run its setup-python.ps1, render something small")
