...
DROP_FROM_HISTORY = ["scripts/make-tester-repo.py", "scripts/make-tester-history.py"]
...
REDACT = [
    row.rstrip("\n").encode() for row, _ in _tester.SCRUB_CLAUDE_MD if row.startswith("|")
] + [
    b"`docs/product-strategy.md`",
    b"`docs/market-shorts-2026.md`",
    b"`docs/shorts-strategy.md`",
    b"`docs/shorts-gtm-playbooks.md`",
    ...
    (b"4149", b"4111"),
    b"4149 4390 2701 0499",
    b"4149439027010499",
    b"4149439027010949",
    b"4441 **** **** 7789",
    b"2527428",
    b"agamanuk@gmail.com",
    b"+38 (066) 317-3125",
    b"+380664134978",
    b"066 431 4978",
    b"0939589090",
    b"\xd0\xa1\xd1\x82\xd1\x80\xd0\xb5\xd0\xbb\xd1\x8c\xd1\x87\xd0\xb5\xd0\xbd\xd0\xba\xd0\xbe",
...
DROPPED_MSG = b"Internal research\n\nThis commit's content is not part of this repository.\n"
...
def git(args, cwd, check=True, capture=True):
    r = subprocess.run(
        ["git"] + args, cwd=cwd, env=_env.clean_env(), capture_output=capture, text=True
    )
    if check and r.returncode:
        sys.exit("git %s failed in %s\n%s" % (" ".join(args[:2]), cwd, (r.stderr or "").strip()))
...
def filter_repo_cmd():
    ...
    for cand in (exe + ".exe", exe):
        if os.path.exists(cand):
            return [cand]
    sys.exit(
        "git-filter-repo is not installed in .venv -- "
...
def verify(outdir):
    ...
    for bad in _tester.FORBIDDEN:
        ...
        if hit.stdout.strip():
            ...
            problems.append("forbidden string %r survives at %s" % (bad, first[:70]))
    for bad in BLOB_TERMS:
        hit = subprocess.run(
            ["git", "grep", "-l", bad.decode()] + revs,
            cwd=outdir,
            env=_env.clean_env(),
            capture_output=True,
            text=True,
        ...
        if hit.stdout.strip():
            first = hit.stdout.strip().splitlines()[0]
            problems.append("business prose %r survives at %s" % (bad.decode(), first[:70]))
    ...
    for rev in revs:
        ...
        for t in MSG_TERMS:
            if t in msg:
                ...
                problems.append(
                    "unreviewed message mentions %r: %s %s" % (t.decode(), rev[:8], subj[:60])
                )
                break
...
def build(outdir):
    if os.path.exists(outdir):
        sys.exit("refusing to build into existing %s -- delete it first" % outdir)
    ...
    subprocess.run(
        filter_repo_cmd()
        + paths
        + ["--replace-text", repl, "--message-callback", msg_cb, "--force"],
        cwd=outdir,
        env=_env.clean_env(),
        check=True,
    )
    ...
    for p in DROP_FROM_HISTORY:
        drop += ["--path", p]
    subprocess.run(
        filter_repo_cmd() + drop + ["--invert-paths", "--force"],
        cwd=outdir,
        env=_env.clean_env(),
        check=True,
    )
...
def main():
    ...
    ap.add_argument(
        "--list",
        action="store_true",
        help="print the plan -- paths kept, text redacted, commits affected -- and do nothing",
    )
    args = ap.parse_args()
    ...
    if args.list or not args.out:
        ...
        print(
            ...
            "subject mentions: %s" % (len(REDACT), ", ".join(t.decode() for t in MSG_TRIGGERS))
        )
        subj = [
            l
            for l in git(["log", "--format=%s"], ROOT).splitlines()
