...
def segment(words, max_dur=4.0, min_dur=0.9):
    ...
    for n, u in enumerate(units):
        if out and _span(u) < min_dur:
            ...
            gap_next = units[n + 1][0]["start"] - u[-1]["end"] if n + 1 < len(units) else 1e9
...
def plan_from_script(clip, path, start, end):
    ...
    if rows[0]["t"] < -1e-6:
        sys.exit("%s: first line starts at %.2fs, before the clip" % (clip["id"], rows[0]["t"]))
    if rows[-1]["t"] >= dur:
        sys.exit(
            "%s: a line starts at %.2fs but the clip is %.2fs long"
...
def _has_rubberband():
    global _RB
    if _RB is None:
        p = subprocess.run(["ffmpeg", "-v", "error", "-filters"], capture_output=True, env=ENV)
...
def _stretch(x, factor):
    ...
    p = subprocess.run(
        [
            ...
            "pipe:0",
            "-filter:a",
            "rubberband=tempo=%.6f" % (1.0 / factor),
            "-f",
            "f32le",
            "pipe:1",
        ],
    ...
    if got > hard:  # last resort, kept small on purpose
        ...
        if f > 0.82:
            ...
            if ok:
                ...
                why = (
                    "this ffmpeg has no rubberband filter"
                    if not _has_rubberband()
                    else "rubberband failed"
                )
                print(
                    "   WARNING: slot %d: %s -- audio left %.2fs over its slot"
                    % (u["i"], why, got - hard)
                )
    final = audio.size / float(SR)
    if final > hard + 0.05:
        print(
            "   WARNING: slot %d overruns its hard budget (%.2fs > %.2fs) -- "
...
def place(units, audios, total):
    ...
    for u, a in zip(units, audios):
        ...
        i = int(round(u["t0"] * SR))
        if i >= bed.size:
            print("   WARNING: slot %d starts past the end of the clip -- skipped" % u["i"])
            continue
        a = a.copy()
    ...
    if tail.size and float(np.abs(tail).max()) > 1e-4:
        ...
        print("   WARNING: %.2fs of audio ran past the clip end and was cut" % over)
    return bed[:n]
...
def keep_better(changed, units_by_i, old, before, audio_by_i, marks_by_i, fit_by_i, by_i):
    ...
    for i in changed:
        u = units_by_i[i]
        if abs(fit_by_i[i]["final"] - u["dur"]) > abs(old[i][2]["final"] - u["dur"]) + 1e-6:
...
def main():
    ...
    ap.add_argument(
        "--el-model",
        help="ElevenLabs model id override (default comes from config/elevenlabs-voices.json)",
    )
    ap.add_argument(
        "--tag",
    ...
    ap.add_argument(
        "--max-dur", type=float, default=4.0, help="longest speech unit before it is split again"
    )
    ...
    ap.add_argument(
        "--plan-only", action="store_true", help="write and print the segmentation, spend nothing"
    )
    ap.add_argument(
        "--retranslate",
        action="store_true",
    ...
    ap.add_argument(
        "--force",
        action="store_true",
        help="re-render even if the wav exists; keeps the cached translation (see --retranslate)",
    )
    args = ap.parse_args()
    ...
    print("voice %s via %s, tag %s" % (_tts.resolve_voice(voice, backend), backend, tag))
    if args.translation and args.retranslate:
        sys.exit(
            "--translation and --retranslate contradict each other: one "
    ...
    if args.script and len(selected) != 1:
        ...
        sys.exit(
            "--script is written for one clip's timeline, but %d clips are "
            "selected -- narrow it with --only" % len(selected)
        )
    if args.translation and len(selected) > 1:
        sys.exit(
            "--translation is one clip's script; select that clip with "
    ...
    for clip in selected:
        ...
        stem = os.path.join(args.outdir, ("%s-%s" % (prefix, clip["id"])) if prefix else clip["id"])
        ...
        if args.plan_only:  # before the skip: re-planning an
            plan = (
                plan_from_script(clip, args.script, start, end)[0]
                if args.script
                else build_plan(
                    clip,
                    words,
                    start,
                    end,  # already-rendered
                    args.max_dur,
                    args.min_dur,
                )
            )  # clip is the point
            ...
            print(
                "   %d speech units, median %.2fs, longest %.2fs"
        ...
        if os.path.exists(dpath):
            ...
            if was != (None, None) and was != now and not args.force:
                sys.exit(
                    "tag %r already holds a %s/%s dub; you asked for %s/%s."
        ...
        def save_rows(rows):
            with open(tpath, "w", encoding="utf-8") as f:
                json.dump({"fingerprint": fp, "rows": rows}, f, ensure_ascii=False, indent=2)
        ...
        if script_rows is not None:
            ...
            if isinstance(data, dict):
                if data.get("fingerprint") not in (None, fp):
                    sys.exit(
                        "%s was translated for a different plan or engine "
                        "(fingerprint %s, this run needs %s).\nPass "
                        "--retranslate, or use a fresh --tag."
                        % (os.path.basename(tpath), data.get("fingerprint"), fp)
                    )
                rows = data["rows"]
            else:
                rows = data  # pre-fingerprint file: trust it once
                print(
                    "   note: %s predates plan fingerprints -- assuming it "
        ...
        def render(indices):
            for i in indices:
                (audio_by_i[i], marks_by_i[i], fit_by_i[i]) = fit_unit(
                    units_by_i[i], by_i[i], voice, backend
                )
        ...
        for rnd in range(args.tune_rounds):
            ...
            if not n:
                break
            by_i = {int(r["i"]): r for r in rows}
            changed = [i for i in sorted(by_i) if by_i[i]["text"] != before[i]["text"]]
            if not changed:
                break
            print("   round %d: re-rendering %d slot(s)" % (rnd + 1, len(changed)))
            render(changed)
            for i in keep_better(
                changed, units_by_i, old, before, audio_by_i, marks_by_i, fit_by_i, by_i
            ):
                print("   slot %d: rewrite fit worse, kept the old line" % i)
            save_rows(rows)
            if args.translation:
                print(
                    "   (retuned rows went to %s; your --translation file "
        ...
        if clamped:
            print(
                "   WARNING: nudged %d word mark(s) forward to keep them "
        ...
        report = {
            ...
            "start": round(start, 3),
            "end": round(end, 3),
            "voice": _tts.resolve_voice(voice, backend),
            "backend": backend,
            ...
            "tight_missing": sum(
                1
                for r in rows
                if not str(r.get("tight", "")).strip() or r["tight"].strip() == r["text"].strip()
            ),
        ...
        print(
            "   sync %.1f%%  slot error mean %.2fs max %.2fs  tight %d  squeezed %d"
            % (
                report["sync"] * 100,
                report["slot_error_mean"],
                report["slot_error_max"],
                report["used_tight"],
                report["squeezed"],
            )
        )
        ...
        _project.record(
            ...
            sidecars={
                "plan": "%s.%s.plan.json" % (stem, tag),
                "translation": "%s.%s.translation.json" % (stem, tag),
                "report": dpath,
                "words": wjson,
            },
            note="sync %.1f%%, %s/%s" % (report["sync"] * 100, backend, report["voice"]),
        )
