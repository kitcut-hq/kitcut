...
DEFAULT_RENDER = {"speed": 5, "cq": 21, "maxrate": "16M", "bufsize": "32M", "audio_bitrate": "192k"}
DEFAULT_CUT = {
    "min_silence": 1.5,
    "air": 0.4,
    "silence_db": -34,
...
DEFAULT_PIP = {
    ...
    "crop_y": 0.5,
    "border_px": 3,
    "border_colour": "#F2F2F2",
    "corner_radius_px": 0,
}
DEFAULT_CANVAS = {"width": 1920, "height": 1080, "fit": "pad", "background": "#000000", "fps": 30}
...
def silent_spans(audio, db, min_dur):
    ...
    return detect_spans(
        [
            ...
            "-vn",
            "-af",
            "silencedetect=noise=%ddB:d=%.3f" % (db, min_dur),
            "-f",
            "null",
            "-",
        ],
...
def frozen_spans(video, db, min_dur=0.5):
    ...
    return detect_spans(
        [
            ...
            "-an",
            "-vf",
            "freezedetect=n=%ddB:d=%.3f" % (db, min_dur),
            "-f",
            "null",
            "-",
        ],
...
def plan_cuts(m, cut, film_a, film_b, screen_dur, offset, fps, verbose=True):
    ...
    if verbose:
        print(
            "  %d silences >= %.1fs at %ddB inside the film"
    ...
    if cut.get("require_frozen", True) or cutaway:
        ...
        if verbose:
            ...
            print(
                "  screen is frozen for %.1fs of its %.1fs (%.0f%%)"
                % (tot, screen_dur, 100.0 * tot / screen_dur)
            )
    if cut.get("require_frozen", True):
        still = merge(fz_cam + [(film_a - 1.0, cov_a), (cov_b, film_b + 1.0)], gap=0.0)
    else:
    ...
    long_still = (
        [(a, b - lead_out) for a, b in fz_cam if b - lead_out - a >= cutaway] if cutaway else []
    )
    ...
    if verbose and cutaway:
        cam = sum(b - a for a, b, l in keeps if l == "full" and cov_a <= a and b <= cov_b)
        print("  %.1fs cuts away to camera (screen still for over %.0fs there)" % (cam, cutaway))
    return keeps, drops
...
def pip_masks(pip, outdir):
    ...
    ImageDraw.Draw(mask).rounded_rectangle(
        [b, b, s - 1 - b, s - 1 - b], radius=max(1, radius - b), fill=255
    )
    ...
    if b > 0:
        ...
        d.rounded_rectangle([0, 0, s - 1, s - 1], radius=radius, outline=col, width=b)
...
def cat(ch, parts, out, v, a):
    ...
    if len(parts) == 1:
        ...
        ch.append("%sconcat=n=%d:v=%d:a=%d[%s]" % ("".join(parts), len(parts), v, a, out))
    return "[%s]" % out


def fill(w, h):
    return "scale=%d:%d:force_original_aspect_ratio=increase:flags=lanczos,crop=%d:%d,setsar=1" % (
        w,
        h,
        w,
        h,
...
def build_graph(
    acts, offset, canvas, pip, audio_cfg, mask_idx, border_idx, label_fc="", label_out=None
):
    ...
    for idx in sorted(vt):
        ...
        ch.append(
            "[%d:v]fps=%d,split=%d%s" % (idx, fps, len(labs), "".join("[%s]" % l for l in labs))
        )
        pool_v[idx] = labs
    for idx in sorted(at):
        labs = ["xa%d_%d" % (idx, i) for i in range(at[idx])]
        ch.append("[%d:a]asplit=%d%s" % (idx, len(labs), "".join("[%s]" % l for l in labs)))
        pool_a[idx] = labs

    npip = sum(1 for a in acts if a["layout"] == "pip")
    if npip and mask_idx is not None:
        ch.append("[%d:v]format=gray,split=%d%s" % (mask_idx, npip, labels("mk", npip)))
    if npip and border_idx is not None:
        ch.append("[%d:v]format=rgba,split=%d%s" % (border_idx, npip, labels("bd", npip)))
    ...
    vfmt = "format=yuv420p"
    afmt = "aresample=48000,aformat=sample_fmts=fltp:sample_rates=48000:channel_layouts=stereo"
    ...
    for i, act in enumerate(acts):
        if act["layout"] == "clip":
            src, a0, b0 = act["audio"]
            ch.append(
                "[%s]atrim=start=%.4f:end=%.4f,asetpts=PTS-STARTPTS,%s[a%d]"
            ...
            for j, (psrc, pa, pb) in enumerate(act["parts"]):
                lab = "cp%d_%d" % (i, j)
                ch.append(
                    "[%s]trim=start=%.4f:end=%.4f,setpts=PTS-STARTPTS,%s,%s[%s]"
        ...
        for a, b in act["segs"]:
            ch.append(
                "[%s]trim=start=%.4f:end=%.4f,setpts=PTS-STARTPTS[tv%d]"
                % (pool_v[1].pop(0), a, b, k)
            )
            ch.append(
                "[%s]atrim=start=%.4f:end=%.4f,asetpts=PTS-STARTPTS[ta%d]"
            ...
            if act["layout"] == "pip":
                ch.append(
                    "[%s]trim=start=%.4f:end=%.4f,"
                    "setpts=PTS-STARTPTS[ts%d]" % (pool_v[0].pop(0), a - offset, b - offset, k)
                )
                sp.append("[ts%d]" % k)
            k += 1
        ...
        ch.append("%s%s,scale=%d:%d:flags=lanczos,setsar=1[sq%d]" % (camv, sq, s, s, i))
        last = "sq%d" % i
        ...
        if mask_idx is not None:
            ...
            last = "pip%d" % i
        out = "vraw%d" % i
        ch.append("[bg%d][%s]overlay=%d:%d:shortest=1[%s]" % (i, last, px, py, out))
        if border_idx is not None:
            ch.append("[%s][bd%d]overlay=%d:%d:shortest=1[vb%d]" % (out, si, px, py, i))
            out = "vb%d" % i
    ...
    ch.append("%sconcat=n=%d:v=1:a=1[%s][araw]" % (pairs, n, "vcat" if label_fc else "vout"))
    if label_fc:
        ch.append(label_fc)
...
def build_bookend(spec, extra):
    ...
    for cut_in in sorted(spec.get("broll") or [], key=lambda c: c["at"]):
        ...
        if at < cur - 1e-6 or at + dur > b0 + 1e-6:
            sys.exit(
                "bookend %s: b-roll at %.2f+%.2f does not fit inside the "
                "clip or overlaps the one before it" % (spec.get("id"), cut_in["at"], dur)
            )
        if at > cur + 1e-6:
        ...
        if bfrom + dur > blen + 1e-6:
            sys.exit(
                "bookend %s: b-roll wants %.2f..%.2f of a %.2fs source"
    ...
    if abs(tiled - (b0 - a0)) > 1e-3:
        sys.exit(
            "bookend %s: picture is %.3fs against %.3fs of sound" % (spec.get("id"), tiled, b0 - a0)
        )

    return {
        "layout": "clip",
        "id": spec.get("id", "bookend"),
        "path": path,
        "audio": (src, a0, b0),
        "parts": parts,
    }
...
def main():
    ...
    ap.add_argument("--plan", action="store_true", help="write the keep-list sidecar and stop")
    ap.add_argument(
        "--list", action="store_true", help="print the timeline and runtime, encode nothing"
    )
    ...
    ap.add_argument(
        "--preview", type=float, default=0.0, help="render only the first N seconds, for a look"
    )
    ...
    for spec in img_specs:
        ...
        if len(given) != 1:
            sys.exit(
                "every image_overlays entry needs exactly one of "
                "%s -- got %s" % ("/".join(_imgoverlay.SOURCES), ", ".join(given) or "none")
            )
        src = _imgoverlay._overlay.repo_path(spec[given[0]])
        if not os.path.exists(src):
            sys.exit("image overlay source does not exist: %s" % src)

    sync_path = os.path.join(os.path.dirname(args.manifest), "%s.sync.json" % mid)
    ...
    print("  film spans camera %.2f .. %.2f  (%.1fs of tape)" % (film_a, film_b, film_b - film_a))

    cuts_path = args.cuts or os.path.join(os.path.dirname(args.manifest), "%s.cuts.json" % mid)
    if args.cuts and os.path.exists(args.cuts):
        ...
        keeps, drops = plan_cuts(m, cut, film_a, film_b, screen_dur, offset, fps)
    ...
    print(
        "  core %s  (cut %.1fs of pauses, %.0f%% of the tape)"
        % (hhmmss(core), dropped, 100.0 * dropped / max(1e-9, film_b - film_a))
    )
    ...
    for a in opens + closes:
        print(
            "  bookend %-10s %6.2fs from %s%s"
            % (
                a["id"],
                act_dur(a),
                os.path.basename(a["path"]),
                "  (+%d b-roll)" % (len(a["parts"]) // 2) if len(a["parts"]) > 1 else "",
            )
        )

    runtime = sum(act_dur(a) for a in acts)
    print("  runtime %s" % hhmmss(runtime))
    print("  acts: %s" % ", ".join("%s (%s)" % (a["layout"], hhmmss(act_dur(a))) for a in acts))
    ...
    if args.list:
        ...
        for a, b, layout in keeps:
            said = ""
            if words:
                ws = [w["text"] for w in words if a <= w["start"] < min(b, a + 6)]
                said = " ".join(ws)[:56]
            print("  %8.2f %8.2f %6.2f   %-6s   %s" % (a, b, b - a, layout, said))
        if drops:
            print()
            print("  cut out:")
            for a, b in drops:
    ...
    cam_to = (
        max([b for act in acts if act["layout"] != "clip" for _, b in act["segs"]] or [1.0]) + 2.0
    )
    scr_ends = [b - offset for act in acts if act["layout"] == "pip" for _, b in act["segs"]]
    inputs = []
    if scr_ends:
    ...
    if extra:
        base = dict(
            (ordinal, nxt + n)
            for n, (_, ordinal) in enumerate(sorted(extra.items(), key=lambda kv: kv[1]))
        )
        for act in acts:
            if act["layout"] != "clip":
                continue
    ...
    graph = build_graph(
        acts, offset, canvas, pip, audio_cfg, mask_idx, border_idx, label_fc, label_out
    )
    ...
    if peak is None or peak < -60:
        sys.exit("rendered audio is silent (peak %s dB); refusing to ship it" % peak)
    ...
    if got_rot:
        sys.exit(
            "output carries rotation=%s, so players will turn it on its "
            "side; %s left in place" % (got_rot, tmp)
        )
    w, h = _dims(tmp)
    if (w, h) != (canvas["width"], canvas["height"]):
        sys.exit("output is %dx%d, expected %dx%d" % (w, h, canvas["width"], canvas["height"]))

    shutil.move(tmp, dst)
    print(
        "  %s  %s  %.1f MB  audio peak %.1f dB"
        % (os.path.relpath(dst, ROOT), hhmmss(got), os.path.getsize(dst) / 1e6, peak)
    )
    ...
    _project.record(
        ...
        burned=(
            [
                "pause cut per cuts.json (min_silence %s)" % cut["min_silence"],
            ...
            + [_imgoverlay.describe(s, img_preset_doc, runtime) for s in img_specs]
        ),
    )
