...
def render_template(tpl, ctx):
    ...
    while True:
        m = re.search(r"\{\{([#^])\s*([\w.]+)\s*\}\}(.*?)\{\{/\s*\2\s*\}\}", tpl, re.DOTALL)
        if not m:
            break
        ...
        if kind == "^":
            ...
            out = "".join(
                render_template(body, dict(ctx, **(v if isinstance(v, dict) else {".": v})))
                for v in val
            )
        elif val:
            out = render_template(body, dict(ctx, **val) if isinstance(val, dict) else ctx)
        else:
            out = ""
...
def list_dir(rel, ext):
    ...
    if not os.path.isdir(d):
        return []
    return sorted(os.path.splitext(f)[0] for f in os.listdir(d) if f.endswith(ext))


def load_brand(name):
    p = (
        _overlay.repo_path(os.path.join(BRAND_DIR, "%s.json" % name))
        if not name.endswith(".json")
        else _overlay.repo_path(name)
    )
...
def load_template(name):
    p = (
        _overlay.repo_path(os.path.join(TEMPLATE_DIR, "%s.html" % name))
...
def build_html(spec, brand=None, template=None):
    ...
    ctx.update({k: v for k, v in spec.items() if k not in ("brand", "template")})
    ...
    for ln in spec.get("lines") or []:
        ...
        for key, prop in (
            ...
            if ln.get(key) is not None:
                v = ln[key]
                css.append("%s:%s" % (prop, "%gpx" % v if isinstance(v, (int, float)) else v))
        ln["inline"] = ";".join(css)
...
def main():
    ap = argparse.ArgumentParser(description="Design a card from a spec, a template and a brand.")
    ...
    ap.add_argument(
        "--list",
        action="store_true",
        help="show the templates, brands and line styles available -- design nothing",
    )
    args = ap.parse_args()
    ...
    if args.list:
        ...
        for t in list_dir(TEMPLATE_DIR, ".html"):
            head = open(
                _overlay.repo_path(os.path.join(TEMPLATE_DIR, t + ".html")),
                encoding="utf-8",
            ).read()
    ...
    out = args.out or (
        os.path.splitext(_overlay.repo_path(args.spec))[0] + ".html"
        if args.spec
        else "temp/card.html"
    )
    ...
    if args.png:
        png = args.png if isinstance(args.png, str) else os.path.splitext(out)[0] + ".png"
        vw, vh = (int(x) for x in args.viewport.replace("x", ",").split(","))
        w, h = _html2img.render(
            out, png, viewport=(vw, vh), scale=args.scale, pad=int(spec.get("pad_px", 8))
        )
